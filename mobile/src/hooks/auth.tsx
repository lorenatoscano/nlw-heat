...
const CLIENT_ID = 'ed2707e1fc38bc964cbd';
...
function AuthProvider({ children }: AuthProviderProps) {
  ...
  async function signIn() {
    try {
      ...
      const { params, type } = await AuthSessions.startAsync({ authUrl }) as AuthorizationResponse;

      if (type === 'success' && params.error !== 'access_denied') {
        const { data } = await api.post('/authenticate', { code: params.code });
